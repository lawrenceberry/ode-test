"""Tests for a synthetic stiff 70D mass-conserving ODE system."""
...
def _stiff_70d_ode(y, p):
    """70D stiff linear chain with conserved total mass.
...
@jax.jit
def _stiff_70d_array(y, p):
    """Array-returning adapter for non-Pallas ensemble reference solver."""
    return jnp.array(_stiff_70d_ode(y, p))


def test_rodas5_v2_70d_matches_rodas5_reference():
    """Validate Pallas v2 solver on a new stiff 70D system."""
    N = 256
    ...
    solve_v2 = make_rodas5_v2_solver(_stiff_70d_ode)
...
@pytest.fixture
def params_batch_70d(request):
    """70D rate-scale parameter sets with +/-10% uniform perturbation."""
    N = request.param
...
@pytest.mark.parametrize(
    "params_batch_70d", [2, 100, 1000, 10000, 100_000], indirect=True
)
def test_rodas5_70d_ensemble_N(benchmark, params_batch_70d):
    """Rodas5 vmap ensemble benchmark on the stiff 70D system."""
    y0 = jnp.array([1.0] + [0.0] * (_N_VARS - 1), dtype=jnp.float64)
    ...
    assert results.shape == (params_batch_70d.shape[0], _N_VARS)
...
def test_rodas5_v2_70d_pallas_ensemble_N(benchmark, params_batch_70d):
    """Rodas5 v2 Pallas custom kernel ensemble benchmark on the stiff 70D system."""
    solve_v2 = make_rodas5_v2_solver(_stiff_70d_ode)
    y0 = jnp.array([1.0] + [0.0] * (_N_VARS - 1), dtype=jnp.float64)
    y0_batch = jnp.broadcast_to(y0, (params_batch_70d.shape[0], _N_VARS))
    results = benchmark.pedantic(
        lambda: solve_v2(
            y0_batch=y0_batch,
            t_span=_T_SPAN,
...
def test_rodas5_v2_70d_pallas_compile_time(benchmark):
