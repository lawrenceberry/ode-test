...
_JULIA_KERNEL_CACHE_KEY = "julia_tsit5_EnsembleGPUKernel"
_JULIA_KERNEL_COLOR = "#9b59b6"
_JULIA_KERNEL_LABEL = "Julia Tsit5 kernel (N=400k, 0% waste)"
...
def time_julia_kernel() -> float:
    _, params = lorenz.make_scenario("divergent", _N_TRAJ)
...
def run_benchmarks(gpu_name: str, cache: dict) -> tuple[list[dict], float | None]:
    ...
    if julia_ms is None:
        print(f"Julia Tsit5 kernel (N={_N_TRAJ}) ...", end=" ", flush=True)
        try:
        ...
        print(f"Julia Tsit5 kernel (N={_N_TRAJ}) ... (cached) {julia_ms:.1f} ms")
