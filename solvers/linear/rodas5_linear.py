    ...
    def _solve_impl(
        ...
        def _solve_batch(params_batch):
            ...
            def _step_batch(y, t, dt):
                # Precompute Jacobians at each distinct stage time.
                # W is factored at t0; f_eval at each stage uses J(t_stage).
                t_end = t + dt
                jac_0 = _jac_at_batched(t, params_batch)
                jac_c2 = _jac_at_batched(t + _c2 * dt, params_batch)
                jac_c3 = _jac_at_batched(t + _c3 * dt, params_batch)
                jac_c4 = _jac_at_batched(t + _c4 * dt, params_batch)
                jac_c5 = _jac_at_batched(t + _c5 * dt, params_batch)
                jac_1 = _jac_at_batched(t_end, params_batch)

                dtgamma_inv = (1.0 / (dt * _gamma)).astype(lu_dtype)[:, None, None]
                lu = lu_factor_batched(dtgamma_inv * eye - jac_0.astype(lu_dtype))
                inv_dt = (1.0 / dt)[:, None]
