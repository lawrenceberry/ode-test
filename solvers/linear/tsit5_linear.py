"""Tsit5 solver — linear ODE variant (jac_fn path).
...
def make_solver(
    ...
    """Create a reusable Tsit5 ensemble solver for linear ODEs.
    ...
    _jac_batched = jax.vmap(jac_fn)
    _jac_one = jax.jit(jac_fn)
        ...
        def _solve_batch(params_batch):
            ...
            def _fresh_k1(y, t, params_batch, k_fsal, has_fsal):
                def _pick_one(y_i, t_i, p_i, k_fsal_i, has_fsal_i):
                    def _compute(_):
                        jac = _jac_one(t_i, p_i).astype(mv_dtype)
                        out = _batched_matvec(
                            jac[None, :, :],
                            y_i.astype(mv_dtype)[None, :],
            ...
            def _step_batch(y, t, dt, k_fsal, has_fsal):
                ...
                def f_eval(u, t_stage):
                    jac = _jac_batched(t_stage, params_batch).astype(mv_dtype)
                    out = _batched_matvec(
                        jac,
                        u.astype(mv_dtype),
                        precision=dot_precision,
